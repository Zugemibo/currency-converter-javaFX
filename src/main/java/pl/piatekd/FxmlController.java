...
public class FxmlController implements Initializable {
	...
	void cbBaseCurrencyValueHandler(ActionEvent event) {
		System.out.println("Succesfully changed base currency to: " + cbBaseCurrency.getValue());
	}

	@FXML
	void cbOutputCurrencyValueHandler(ActionEvent event) {
		System.out.println("Succesfully changed output currency to: " + cbOutputCurrency.getValue());
	}
	...
	private void handleButtonAction(ActionEvent event) {
		if (event.getSource() == btnClose) {
			...
			String message = tfQuantity.getText() + 
					cbBaseCurrency.getValue() + 
					" in " + 
					cbOutputCurrency.getValue() + 
					" = " + 
					calculated;
			calculatedValue.setText(message);
		}
	}
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		cbBaseCurrency.getItems().setAll(CurrencyShortName.values());
		cbOutputCurrency.getItems().setAll(CurrencyShortName.values());
		try {
			getData();
		} catch (IOException e) {
			e.printStackTrace();
